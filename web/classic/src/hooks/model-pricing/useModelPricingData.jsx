...
import {
  ...
  marketplaceGroupMatchesModel,
} from '../../components/table/model-pricing/group-display';
...
export const useModelPricingData = () => {
  ...
  const [filterQuotaType, setFilterQuotaType] = useState('all'); // 计费类型筛选: 'all' | 0
  ...
  const filteredModels = useMemo(() => {
    ...
    if (filterGroup !== 'all') {
      result = result.filter((model) =>
        marketplaceGroupMatchesModel(model, filterGroup),
      );
    }
    ...
    if (group === 'all') {
      ...
      showInfo(
        t('当前查看的分组为：{{group}}，倍率为：{{ratio}}', {
          group: getMarketplaceGroupLabel(group, t),
