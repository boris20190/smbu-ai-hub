...
const PersonalSetting = () => {
  ...
  const refreshEmailVerificationCaptcha = useCallback(async () => {
    setCaptchaLoading(true);
    try {
      const res = await API.get('/api/verification/captcha', {
        disableDuplicate: true,
      });
      const { success, message, data } = res.data;
      if (success && data?.captcha_id && data?.image) {
        setCaptchaChallenge(data);
        setCaptchaCode('');
      } else {
        setCaptchaChallenge(null);
        showError(message || t('加载图形验证码失败，请重试'));
      }
    } catch (error) {
      setCaptchaChallenge(null);
      showError(t('加载图形验证码失败，请重试'));
    } finally {
  ...
  useEffect(() => {
    if (showEmailBindModal) {
      refreshEmailVerificationCaptcha();
    } else {
      setCaptchaChallenge(null);
      setCaptchaCode('');
    }
  }, [showEmailBindModal, refreshEmailVerificationCaptcha]);
  ...
  const sendVerificationCode = async () => {
    ...
    if (!captchaChallenge?.captcha_id) {
      showError(t('图形验证码不可用，请刷新后重试'));
      await refreshEmailVerificationCaptcha();
      return;
    }
    if (captchaCode.trim() === '') {
      showError(t('请先输入图形验证码！'));
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <div className='mt-[60px]'>
      ...
      <EmailBindModal
        ...
        refreshEmailVerificationCaptcha={refreshEmailVerificationCaptcha}
        countdown={countdown}
        turnstileEnabled={turnstileEnabled}
        turnstileSiteKey={turnstileSiteKey}
