...
import {
  getMarketplaceGroupLabel,
  normalizeMarketplaceGroup,
} from '../../model-pricing/group-display';
...
const EditTokenModal = (props) => {
  ...
  const loadGroups = async () => {
    ...
    if (success) {
      ...
      let localGroupOptions = Object.entries(data).reduce(
        (options, [group, info]) => {
          const value = normalizeMarketplaceGroup(group);
          if (!value || seenGroups.has(value)) {
            return options;
          }
          seenGroups.add(value);
          options.push({
            label: getMarketplaceGroupLabel(value, t),
            value,
            ratio: info.ratio,
          });
          return options;
  ...
  const renderTokenGroupOption = (item) => {
    const { disabled, selected, label, value, focused, onMouseEnter, onClick } =
      item;
    ...
    return (
      <div
        ...
        onMouseEnter={(event) => {
          if (!disabled && onMouseEnter) onMouseEnter(event);
        }}
      >
        <Text strong type={disabled ? 'tertiary' : undefined}>
          {label || value}
        </Text>
        {item.ratio ? (
  ...
  const loadToken = async () => {
    ...
    if (success) {
      ...
      data.group = normalizeMarketplaceGroup(data.group) || 'offline';
                  ...
                  <Col span={24}>
                    {groups.length > 0 ? (
                      <Form.Select
                        ...
                        optionList={groups}
                        renderOptionItem={renderTokenGroupOption}
                        filter={(input, option) => {
