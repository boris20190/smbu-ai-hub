...
import type {
  ...
  RegisterPayload,
  ApiResponse,
  EmailVerificationCaptchaResponse,
} from './types'
...
// Get manual captcha challenge for email verification
export async function getEmailVerificationCaptcha(): Promise<EmailVerificationCaptchaResponse> {
  const config: ExtendedApiConfig = {
    disableDuplicate: true,
  }
  const res = await api.get('/api/verification/captcha', config)
  return res.data
}
