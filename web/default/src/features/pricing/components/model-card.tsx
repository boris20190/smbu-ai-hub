...
export const ModelCard = memo(function ModelCard(props: ModelCardProps) {
  ...
  const groups = getMarketplaceDisplayGroups(props.model)
  const endpoints = props.model.supported_endpoint_types || []
  const metadata = inferModelMetadata(props.model)
  const contextLabel = formatTokenCount(metadata.context_length)
      ...
      <div className='mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 sm:mt-4'>
        {primaryGroup && (
          <span className='text-muted-foreground text-xs font-medium'>
            {getMarketplaceGroupLabel(primaryGroup, t)} {t('Groups')}
          </span>
        )}
        <span className='text-muted-foreground text-xs font-medium'>
          {t('Token-based')}
