...
import { getMarketplaceGroupLabel } from '../lib/group-display'
...
export function LatencyTrendChart(props: {
  ...
  const spec = useMemo(() => {
    ...
    const data = props.series.map((point) => ({
      ...
      group: getMarketplaceGroupLabel(point.group, t, props.modelName),
      ttft: point.ttft_ms,
    }))
    return {
      type: 'line' as const,
  ...
  }, [props.series, props.modelName, t])
      ...
      data: [
        {
          ...
          values: filtered.map((r) => ({
            ...r,
            group: getMarketplaceGroupLabel(r.group, t, props.modelName),
          })),
        },
      ],
