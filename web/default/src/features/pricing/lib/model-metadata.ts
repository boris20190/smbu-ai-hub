...
const GPT_5_CONTEXT_OUTPUTS: Array<{
  pattern: RegExp
  context: number
  maxOutput: number
}> = [
  { pattern: /^gpt-5\.5(?:-|$)/, context: 1_050_000, maxOutput: 128_000 },
  {
    pattern: /^gpt-5\.4-(?:mini|nano)(?:-|$)/,
    context: 400_000,
    maxOutput: 128_000,
  },
  { pattern: /^gpt-5\.4(?:-|$)/, context: 1_050_000, maxOutput: 128_000 },
  { pattern: /^gpt-5\.3-chat(?:-|$)/, context: 128_000, maxOutput: 16_384 },
  {
    pattern: /^gpt-5\.3(?:-codex)?(?:-|$)/,
    context: 400_000,
    maxOutput: 128_000,
  },
  { pattern: /^gpt-5\.2(?:-codex)?(?:-|$)/, context: 400_000, maxOutput: 128_000 },
  { pattern: /^gpt-5(?:-codex|-mini|-nano|-|$)/, context: 400_000, maxOutput: 128_000 },
]
...
function inferContextAndOutputs(
  ...
  const gpt5Override = GPT_5_CONTEXT_OUTPUTS.find(({ pattern }) =>
    pattern.test(lower)
  )
  if (gpt5Override) {
    return {
      context: gpt5Override.context,
      maxOutput: gpt5Override.maxOutput,
    }
  }
