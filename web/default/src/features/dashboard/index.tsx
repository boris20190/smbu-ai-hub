...
import { ApiKeysPanel } from './components/overview/api-keys-panel'
import { QuotaRedemptionPanel } from './components/overview/quota-redemption-panel'
...
function LogStatCardsFallback({
  ...
  const content = (
    <div
      className={`divide-border/60 grid h-full grid-cols-2 divide-x ${gridClass}`}
    >
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className='px-4 py-3.5 sm:py-3'>
...
export function Dashboard() {
  ...
  const dashboardActions =
    activeSection === 'models' || activeSection === 'overview' ? (
      <>
  ...
  return (
    <SectionPageLayout>
      ...
      {activeSection === 'overview' && dashboardActions != null && (
        <SectionPageLayout.Actions>{dashboardActions}</SectionPageLayout.Actions>
      )}
      <SectionPageLayout.Content>
        <div className='space-y-3 sm:space-y-4'>
          ...
          {activeSection === 'overview' && (
            <>
              <FadeIn className='overflow-hidden rounded-lg border'>
                <div className='divide-border/60 grid grid-cols-1 divide-y lg:grid-cols-2 lg:divide-x lg:divide-y-0'>
                  <SummaryCards embedded compact />
                  <Suspense
                    fallback={<LogStatCardsFallback count={3} embedded />}
                  >
                    <LazyLogStatCards
                      filters={modelFilters}
                      visibleKeys={['tokens', 'avgRpm', 'avgTpm']}
