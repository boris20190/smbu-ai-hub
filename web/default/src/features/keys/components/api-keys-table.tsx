...
type ApiKeysTableVariant = 'page' | 'overview'

type ApiKeysTableProps = {
  variant?: ApiKeysTableVariant
}

type ApiKeysTableState = {
  globalFilter?: string
...
function useLocalApiKeysTableState(defaultPageSize: number): ApiKeysTableState {
  ...
  const ensurePageInRange: ApiKeysTableState['ensurePageInRange'] = (
    pageCount,
    opts = { resetTo: 'first' }
  ) => {
    setPagination((prev) => {
      if (pageCount <= 0 || prev.pageIndex < pageCount) return prev
      return {
        ...prev,
        pageIndex: opts.resetTo === 'last' ? Math.max(0, pageCount - 1) : 0,
      }
    })
  }
...
export function ApiKeysTable(props: ApiKeysTableProps = {}) {
  if (props.variant === 'overview') {
    return <OverviewApiKeysTable />
  }
  return <RoutedApiKeysTable />
}

function RoutedApiKeysTable() {
  ...
  return <ApiKeysTableContent tableState={tableState} variant='page' />
}

function OverviewApiKeysTable() {
  const tableState = useLocalApiKeysTableState(4)

  return <ApiKeysTableContent tableState={tableState} variant='overview' />
}
...
function ApiKeysTableContent({
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: [
      variant === 'overview' ? 'overview-keys' : 'keys',
  ...
  return (
    <DataTablePage
      ...
      toolbarProps={{
        ...
        className: variant === 'overview' ? 'gap-2' : undefined,
      }}
      ...
      bulkActions={
        variant === 'page' ? <DataTableBulkActions table={table} /> : undefined
      }
      showPagination={
        variant === 'page' || (data?.total ?? 0) > pagination.pageSize
      }
      paginationInFooter={variant === 'page'}
      tableClassName={
        variant === 'overview'
      ...
      className={
        variant === 'overview' ? 'flex min-h-0 flex-1 flex-col' : undefined
      }
      tableHeaderClassName={
        variant === 'overview' ? 'bg-background sticky top-0 z-10' : undefined
      }
    />
  )
}
