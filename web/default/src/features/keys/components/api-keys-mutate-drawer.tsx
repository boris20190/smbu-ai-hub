...
} from '@/features/pricing/lib/group-display'
...
export function ApiKeysMutateDrawer({
  ...
  const groups: ApiKeyGroupOption[] = Object.entries(groupsRaw).reduce(
    (options, [key, info]) => {
      const value = normalizeMarketplaceGroup(key)
      if (!value || options.some((option) => option.value === value)) {
        return options
      }
      const label = getMarketplaceGroupLabel(value, t) || value
      const usesMarketplaceLabel = label !== value

      options.push({
        value,
        label,
        desc: usesMarketplaceLabel ? undefined : info.desc || key,
        ratio: info.ratio,
      })
      return options
    },
    [] as ApiKeyGroupOption[]
  )
