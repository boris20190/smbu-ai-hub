...
const NON_TRANSLATABLE_TEXTS = new Set([
  'AI Proxy',
  'AIGC2D',
  'Anthropic',
  'API2GPT',
...
const TECHNICAL_LABEL_WORDS = new Set([
  'AccessKey',
  'AI',
  'API',
  'Client',
...
function isNonTranslatableText(text) {
  const s = text.trim()
  if (!s) return true
  if (NON_TRANSLATABLE_TEXTS.has(s.replace(/:$/, ''))) return true
  if (/^(https?:\/\/|socks5:\/\/|\/[\w./-]+\/?$)/.test(s)) return true
  if (/^[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}$/.test(s)) return true
  if (/^[^<]+&lt;[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}&gt;$/.test(s)) return true
  if (/^[\w.-]+\.[A-Za-z]{2,}(?:&#10;[\w.-]+\.[A-Za-z]{2,})*$/.test(s)) return true
  if (/^[\[{]/.test(s) || /^[{"]/.test(s) || /^".+":/.test(s)) return true
  if (/^[a-z0-9_.-]+$/.test(s)) return true
  if (/^[A-Z0-9_.* -]+$/.test(s)) return true

  const words = s.replace(/:$/, '').split(/\s*\/\s*|\s+/).filter(Boolean)
  return words.length > 0 && words.every((word) => TECHNICAL_LABEL_WORDS.has(word))
}
...
function isLikelyUntranslated({ locale, baseValue, value }) {
  ...
  if (isNonTranslatableText(s)) return false
