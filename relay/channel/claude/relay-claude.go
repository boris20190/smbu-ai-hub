...
func buildClaudeMediaMessage(c *gin.Context, mediaMessage dto.MediaContent) (*dto.ClaudeMediaMessage, error) {
	...
	if mediaMessage.Type == dto.ContentTypeFile {
		...
		mimeHint = mimeTypeFromFileName(file.FileName)
		source = types.NewFileSourceFromData(file.FileData, mimeHint)
	}
	if source == nil {
		return nil, nil
	}
		...
		if message.Role == "system" {
			...
			if message.Role == "tool" {
				...
				for _, mediaMessage := range message.ParseContent() {
					...
					default:
						claudeMediaMessage, err := buildClaudeMediaMessage(c, mediaMessage)
						if err != nil {
